...
# -------------------------
# Model Loading with Chunked Download
# -------------------------
@st.cache_resource
def load_model():
    try:
        # Download with chunking for large files (BEiT v2 is ~1.2GB)
        with st.spinner("Downloading BEiT v2 Large model (~1.2GB, first run only)..."):
            ...
            # Combine chunks and load
            model_data = b''.join(chunks)
            st.info("Loading model into memory...")
            
        checkpoint = torch.load(BytesIO(model_data), map_location='cpu', weights_only=False)
        ...
        st.info("💡 **Troubleshooting tips:**\n- Check your internet connection\n- Try refreshing the page\n- The model is large (~1.2GB) and may take time on slow connections")
        return None
    except Exception as e:
        st.error(f"Error loading model: {e}")
        st.info("Please try refreshing the page. If the problem persists, the model file may be corrupted.")
        return None
